...
- **⚡ Workflows + 🛍 Marketplace** — named, reusable **agent recipes**, one click to run on the current project (with checkpoints, verification, policy, and budget applied). Built-ins cover **release readiness**, **security remediation**, **migration** (from → to), an **incident assistant** (paste a log, it root-causes and fixes), raising test coverage, and updating dependencies. A **Catalog** of ready-made recipes (document, performance pass, accessibility audit, dockerize, API-from-spec, dead-code cleanup) installs with one click; **author your own** in-app (with `{param}` slots), and **Import/Export** `.toml` files to share recipes with your team. Everything persists to `<config>/kestrel/workflows.toml`.
- **🔗 Multi-repo workspace** — link sibling repositories from the Explorer so the agent can reason **across** them: it can `list_repos`, `search` a linked repo by name, and read any linked repo by path (e.g. trace a call from your app into a shared library). Links persist to `.kestrel/workspace.json`; writes stay in the primary project.
