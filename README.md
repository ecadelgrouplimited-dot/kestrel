...
- **Agent mode** — tick **Agent · write files** in the chat bar and the button becomes **Build**: your request is turned into a real set of files written into the *current project*. The model returns a strict file manifest, Kestrel parses it and writes each file under the project root (refusing any path that escapes it via `..` or an absolute path), then reloads the explorer and reports exactly which files it wrote. Turn on **Include project context** too and the agent extends the existing code instead of starting fresh. This is the single-shot wedge of Kestrel's agentic loop — describe a portfolio site, get the files; the iterative read-edit-verify loop builds on top of this.
