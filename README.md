...
- **⚡ Workflows** — named, reusable **agent recipes**, one click to run on the current project (with checkpoints, verification, policy, and budget applied). Built-ins cover **release readiness**, **security remediation**, **migration** (from → to), an **incident assistant** (paste a log, it root-causes and fixes), raising test coverage, and updating dependencies. Add your own — they persist to `<config>/kestrel/workflows.toml`, a shareable file that seeds a workflow marketplace.
